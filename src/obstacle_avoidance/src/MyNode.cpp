...
AvoidObstacleClass::AvoidObstacleClass(int argc, char **argv, std::string node_name) {
    // Setup NodeHandles
    ...
    laser_scan_subscriber = nh->subscribe("/scan", 10, &AvoidObstacleClass::lidarCallback, this);
...
avoidance AvoidObstacleClass::moveToAvoidCollision(float lidar_distance, float lidar_angle) {
...
bool AvoidObstacleClass::isLeftSideOfGoal() {
	...
	if (lidar_array_size != 0) {
		...
		while ((lidar_data.ranges[i] == NAN) || (lidar_data.ranges[i] <= 0.03) || (lidar_data.ranges[i] >= 5)
		 && (i < (lidar_array_size - 30))) {	
			i++;
		}
			...
			while (i < (lidar_array_size- 30)) {
				if ((lidar_data.ranges[i] != NAN) && (lidar_data.ranges[i] > 0.03) &&
				 (lidar_data.ranges[i] < 5)) {
					...
					//Cosine Rule
					double dist_between_pts = sqrt(pow(a, 2) + pow(b, 2) - 2*a*b*cos(C));
					if ((dist_between_pts > MIN_GOAL_WIDTH) && (dist_between_pts < MAX_GOAL_WIDTH)) {
	...
	for(int i = 0; i < sizeOfLidarArray(lidar_data);i++) {
		if ((lidar_data.ranges[i] != NAN) && (lidar_data.ranges[i] > 0.03) &&
		 (lidar_data.ranges[i] < 5)) {
			...
			move_to_avoid_obstacle = moveToAvoidCollision(lidar_data.ranges[i], 
			...
			if (move_to_avoid_obstacle != no_avoidance) {
				if (lidar_data.ranges[i] < critical_distance) {
					...
					if (move_to_avoid_obstacle == left) {
						...
						ang_velocity = (MIN_CRITICAL_DISTANCE-critical_distance)*ANG_VEL_GAIN;
...
void AvoidObstacleClass::lidarCallback(const sensor_msgs::LaserScan::ConstPtr & laser_scan_message) {
...
void AvoidObstacleClass::publishVel() {
...
float AvoidObstacleClass::getLinearVelMsg() {
	return vel_msg.linear.x;
}
	
float AvoidObstacleClass::getAngVelMsg() {
	return vel_msg.angular.z;
}
