...
int main(int argc, char **argv) {
  ...
  while (ros::ok()) {
    ...
    std::cout << "1st action? " << first_action << " 1st no obs? "
              << first_period_no_obs << " start left? " << started_on_left
              << " ang velocity " << ang_velocity << std::endl;
  ...
  while (ros::ok()) {
          ...
          std::cout << "1st action? " << first_action << " 1st no obs? " <<
  first_period_no_obs
           << " start left? " << started_on_left << " ang velocity " <<
  ang_velocity << std::endl;
          //std::cout << " start left? " << started_on_left << std::endl;
          ros::spinOnce();
  loop_rate.sleep();
  } */
