...
protected:
    virtual void SetUp(){
        test_publisher = nh_.advertise<geometry_msgs::Twist>("/scan", 1); //Send laser msgs to node
        test_subscriber = nh_.subscribe("/my_node/cmd_vel", 1, &MyNodeTest::callback, this); //Check velocity messages from node
...
public:
    void callback(const geometry_msgs::Twist::ConstPtr& vel){
        vel_msg.linear.x = vel->linear.x;
        vel_msg.angular.z = vel->angular.z;
    }
};

TEST_F(MyNodeTest, avoidObstacles){

    // publishes laserscan message to the test node
    laser_msg.angle_min=0;
    test_publisher.publish(laser_message);
