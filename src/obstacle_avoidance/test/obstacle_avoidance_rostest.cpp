...
protected:
    virtual void SetUp(){
        ...
        //test_subscriber = nh_.subscribe("/scan", 10, &MyNodeTest::lidarCallback_, this);
		test_publisher = nh_.advertise<sensor_msgs::LaserScan>("/scan", 10);
        test_subscriber = nh_.subscribe("/cmd_vel", 10, &MyNodeTest::velCallback_, this);
...
public:
	...
	void AvoidObstacleClass::velCallback_(const geometry_msgs::Twist::ConstPtr & vel_message) {
    ...
    /*void AvoidObstacleClass::lidarCallback_(const sensor_msgs::LaserScan::ConstPtr & laser_scan_message) {
